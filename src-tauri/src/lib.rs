...
fn set_window_height(window: tauri::WebviewWindow, height: u32) -> Result<(), String> {
    ...
    match window.set_size(Size::Logical(new_size)) {
        Ok(_) => {
            // Don't automatically reposition - preserve user's chosen position
            Ok(())
        }
...
fn reset_window_position(window: tauri::WebviewWindow) -> Result<(), String> {
    match window::position_window_top_center(&window, 54) {
        Ok(_) => Ok(()),
        Err(e) => Err(format!("Failed to reset window position: {}", e))
    }
}
